...
export class BookingController {
  async availability(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const resource = await prisma.bookableResource.findUnique({ where: { assetId: String(assetId) } });
      if (!resource) {
        sendSuccess(res, "Booking availability fetched successfully", { unavailableHours: [] });
        return;
      }
  ...
  async create(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const booking = await prisma.$transaction(async (tx) => {
        ...
        if (conflict) {
          const error: any = new Error("This asset is already booked during the selected period");
      ...
      sendSuccess(res, "Asset booked successfully", booking, 201);
    } catch (error) {
